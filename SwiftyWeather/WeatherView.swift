...
struct WeatherView: View {
    ...
    //Get Data can't be called without an instance of the class weatherVM in this View
    ...
    var body: some View {
        ...
        .task { //On appear could be used but this is an async function so .task is preferred
            await callWeatherAPI()
...
// Putting the functions in an extension isn't necessary but because they are big it makes things easier
extension WeatherView {
    ...
    func getWeekDay(value: Int) -> String {
        // Increase the date by "value" days
        let date = Calendar.current.date(byAdding: .day, value: value, to: Date.now)!
        // Find the day number 1 = Sunday... 7 = Saturday
        let dayNumber = Calendar.current.component(.weekday, from: date)
        // Convert day number to the weekday & return that String
        let weekday = Calendar.current.weekdaySymbols[value-1] // Remember arrays are zero indexed, and we got a dayNumber 1-7
